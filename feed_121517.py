...
class Feed121517(BaseControlModule):
  ...
  def __init__(self):
    ...
    self.min = -20
    self.min_if_no_circulation = 0.75 * self.min
    self.pump_pwm = PumpPWM(self.plc, control_bws_name, control_value_name, self.min)
  ...
  def _set_module_parameters(self, params):
    ...
    # deprecated:
    self.pump_pwm.pwm.period = params.get('pwm_period', self.pump_pwm.pwm.period)
    self.min = params.get('min', self.min)
    # deprecated:
    self.pump_pwm.control_range[1] = params.get('max', self.pump_pwm.control_range[1])
  ...
  def _control_action(self, now):
    ...
    diagnostics |= self.pump_pwm.update(now, control_output * dt if dt else 0)
